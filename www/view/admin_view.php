      ...
      <table class="table table-bordered text-center">
        ...
        <tbody>
          ...
          <tr class="<?php print(is_open($item) ? '' : 'close_item'); ?>">
            <td><img src="<?php print(IMAGE_PATH . entity_str($item['image']));?>" class="item_image"></td>
            <td><?php print(entity_str($item['name'])); ?></td>
            <!-- number_format関数で数字を3桁ごとにカンマ区切りに -->
            <td><?php print(number_format(entity_str($item['price']))); ?>円</td>
            <td>
              <form method="post" action="admin_change_stock.php">
                <div class="form-group">
                  <!-- sqlインジェクション確認のためあえてtext -->
                  <input  type="text" name="stock" value="<?php print(entity_str($item['stock'])); ?>">
                  個
                </div>
